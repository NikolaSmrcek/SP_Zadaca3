...
void Upis_klijenata(){
     ...
     cin>>x.tekuci;
     cout<<"Unesite stanje deviznog racuna klijenta ( EUR - euri): "<<endl;
     cin>>x.devizni;
     cout<<"Zelite li jos dodavati zapise o klijentima? (d = DA  /  n = NE):"<<endl;
     cin>>jos;  
     EnQueueQ(x,red);
     preostalo--;      
...
cout << "godina: " <<tekuci.godina << endl;
cout << "tekuci: " << tekuci.tekuci << endl;
cout << "devizno: " <<tekuci.devizni << endl<<endl<<"__________________"<<endl;
EnQueueQ(tekuci,red);
}
     }
...
void zad_d(){
     ...
     while(pom_d!=0){
                   ...
                   tekuci=FrontQ(red);
                   DeQueueQ(red);
                   if((tekuci.ci==4 && tekuci.godina<1972 && tekuci.di==1 && tekuci.tekuci<100)||(tekuci.ci==1 && tekuci.di==2 && (tekuci.tekuci+(tekuci.devizni*7.4))>5000))prebaci_c++;
                   else EnQueueQ(tekuci,red);
                   }
     cout<<endl<<"*=*=*= Prikaz stanja glavnog reda! =*=*=*"<<endl<<endl;
     Sort_b();
